...
export function MerchandiseDetailClient({ product, config }: { product: MerchandiseProduct; config: GlobalConfig }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 pb-14 sm:pb-16">
      <Link href="/merchandise" className="inline-flex items-center gap-1.5 text-xs sm:text-sm text-neutral-400 hover:text-primary mb-5 sm:mb-8 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
      ...
      <div className="grid lg:grid-cols-[minmax(0,540px)_minmax(340px,420px)] justify-center gap-7 sm:gap-8 lg:gap-10">
            ...
            <div className="flex gap-2 sm:gap-3 mt-3 sm:mt-4 overflow-x-auto pb-1" aria-label="Pilih foto produk">
              {images.map((image, index) => (
                <button
                  ...
                  className={'w-16 h-16 sm:w-20 sm:h-20 rounded-lg sm:rounded-xl overflow-hidden shrink-0 border-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ' + (selectedImage === index ? 'border-primary' : 'border-transparent opacity-70 hover:opacity-100')}
                >
        ...
        <section className="lg:sticky lg:top-24 lg:self-start">
          <div className="flex flex-wrap items-center gap-2 text-[10px] sm:text-[11px] font-semibold">
            ...
            {eventTitle && <span className="px-2.5 py-1 rounded-full bg-primary-soft text-primary">🎭 {eventTitle}</span>}
            {product.is_featured && <span className="inline-flex items-center gap-1 text-primary"><Sparkles className="w-3.5 h-3.5 fill-primary" />Unggulan</span>}
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {product.variants.map((item) => (
                  <button
                    ...
                    className={'min-w-11 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold border focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ' + (variant === item ? 'bg-primary text-primary-foreground border-primary' : 'bg-neutral-900 text-neutral-300 border-neutral-800 hover:border-primary/50')}
                  >
                    {item}
                  </button>
                ))}
          ...
          <div className="flex items-center justify-between gap-4 border-y border-neutral-800 py-4 sm:py-5">
            ...
            <div className="flex items-center gap-2 sm:gap-3 rounded-xl border border-neutral-800 bg-neutral-900 p-1">
              <button type="button" aria-label="Kurangi jumlah" onClick={() => setQuantity((value) => Math.max(1, value - 1))} disabled={quantity <= 1} className="w-9 h-9 grid place-items-center rounded-lg hover:bg-neutral-800 disabled:opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Minus className="w-4 h-4" /></button>
              <span className="w-7 text-center text-sm font-bold font-mono">{quantity}</span>
              <button type="button" aria-label="Tambah jumlah" onClick={() => setQuantity((value) => Math.min(product.stock, value + 1))} disabled={quantity >= product.stock} className="w-9 h-9 grid place-items-center rounded-lg hover:bg-neutral-800 disabled:opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
