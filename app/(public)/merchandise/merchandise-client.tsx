...
export function MerchandiseClient({ products, events }: { products: MerchandiseProduct[]; events: Array<{ id: string; title: string }>; config: GlobalConfig }) {
  ...
  return <div className="min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-14 pb-20">
    ...
    {filteredProducts.length === 0 ? <div className="py-20 text-center border border-neutral-800 bg-neutral-900/20 rounded-3xl"><ShoppingBag className="w-10 h-10 text-neutral-600 mx-auto mb-3" /><p className="text-sm text-neutral-400">Belum ada merchandise pada pilihan ini.</p><button type="button" onClick={() => { setSelectedCategory('Semua'); setSelectedEventId('all'); }} className="text-xs text-primary hover:underline mt-3">Lihat semua koleksi</button></div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{filteredProducts.map((product, index) => { const image = getMerchandiseImages(product.image_urls, product.image_url)[0]; return <motion.div key={product.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: index * 0.04 }}><Link href={`/merchandise/${product.slug}`} className="group h-full rounded-2xl border border-neutral-800 bg-neutral-900/30 hover:bg-neutral-900/60 overflow-hidden flex flex-col transition-all hover:border-primary/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"><div className="relative aspect-square bg-neutral-950 overflow-hidden"><img src={getMerchandiseImageUrl(image)} alt={product.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" /><div className="absolute top-3 left-3 right-3 flex justify-between">{product.is_featured ? <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-primary text-primary-foreground"><Sparkles className="w-3 h-3 fill-primary-foreground" />UNGGULAN</span> : <span />}{product.stock <= 0 && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-red-500 text-white">HABIS</span>}</div></div><div className="p-4 sm:p-5 flex-1 flex flex-col justify-between"><div><div className="flex justify-between gap-2 mb-1.5 text-[10px] font-semibold uppercase tracking-wider"><span className="text-neutral-400">{product.category}</span>{getProductEventTitle(product.events) && <span className="text-primary truncate">🎭 {getProductEventTitle(product.events)}</span>}</div><h2 className="text-sm sm:text-base font-bold text-white group-hover:text-primary line-clamp-1">{product.name}</h2>{product.description && <p className="text-xs text-neutral-400 mt-1 line-clamp-2 leading-relaxed">{product.description}</p>}</div><div className="mt-4 pt-3 border-t border-neutral-800 flex items-center justify-between"><span className="text-sm font-bold text-white font-mono">{formatRupiah(product.price)}</span><span className="inline-flex items-center gap-1 text-xs font-semibold text-white group-hover:text-primary">Lihat detail <ChevronRight className="w-3.5 h-3.5" /></span></div></div></Link></motion.div>; })}</div>}
  </div>;
}
