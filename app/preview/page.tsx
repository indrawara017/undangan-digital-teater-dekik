...
export default function PreviewPage() {
  return (
    <Suspense fallback={<FullScreenLoader text="Mempersiapkan Pratinjau..." />}>
      <PreviewContent />
    </Suspense>
  );
}
...
function PreviewContent() {
  ...
  const getAssetUrl = (fileName: string) => {
    return supabase.storage.from('assets').getPublicUrl(`${eventId}/${fileName}`).data.publicUrl;
  };
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      ...
      const imagesToPreload = [
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/background.jpg`,
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/design.jpg`,
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/ticket.jpg`
      ];
          ...
          {logos.length > 0 && (
            ...
            >
              {logos.map((logo) => (
                <img 
                  key={logo.name}
                  src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/logos/${logo.name}`}
                  alt="Logo"
    ...
    <div className="bg-black text-white min-h-screen selection:bg-neutral-800 relative">
      ...
      <div className="relative z-10">
        <main className="relative min-h-screen w-full overflow-hidden flex flex-col items-center justify-center">
          ...
          >
            <span className="text-[10px] md:text-[11px] uppercase tracking-[0.2em] text-neutral-400 text-center w-full">Kepada Yth.</span>
            <span className="text-xl md:text-2xl font-cormorant font-medium text-white text-center w-full">[Nama Tamu Undangan]</span>
          </motion.div>
        ...
        <div className="fixed top-6 left-6 z-50 bg-black/50 backdrop-blur-md border border-white/20 text-white text-[10px] uppercase tracking-widest px-4 py-2 rounded-full">
          Mode Preview
        </div>
      </main>
      ...
      >
        ...
        <motion.div variants={premiumScaleIn} className="flex-1 w-full max-w-[220px] md:max-w-[280px] mx-auto">
          <div 
            onClick={() => setZoomedImage(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/design.jpg?t=${Date.now()}`)}
      ...
      {eventData?.is_audio_enabled !== false && (
        <FloatingAudioPlayer audioUrl={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/assets/${eventId}/music.mp3?t=${audioTimestamp}`} autoPlay={wantsMusic} />
      )}
